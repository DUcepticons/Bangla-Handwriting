...
for quality_data in os.listdir(LOCATION): 
    ...
    model = load_model('letter-models/resnet50/'+quality_data[0]+'_resnet_model.h5')

    print('Testing on unseen data:')
    x_test = applications.resnet.preprocess_input(tst_img_data)
    y_test = tst_lbl_data

    with open('reports/resnet50/'+quality_data[0]+'_resnet50_report.csv',mode='w', newline='') as csv_file:
