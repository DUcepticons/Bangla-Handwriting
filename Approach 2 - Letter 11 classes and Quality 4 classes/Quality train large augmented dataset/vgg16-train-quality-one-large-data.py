...
callbacks=[ModelCheckpoint(filepath="checkpoints/vgg16_quality_predict.{epoch:02d}-{val_loss:.2f}.hdf5", monitor='val_loss', verbose=1, mode='auto', period=2),tensorboard_callback]
'''
...
model.fit_generator(generator=my_training_batch_generator, steps_per_epoch = int(77105 // batch_size), epochs=10 ,verbose=1,  callbacks=callbacks,  validation_data = my_validation_batch_generator, validation_steps = int(19277 // batch_size)) #will try with 5 epochs later
...
model.save('vgg16_model_quality_one_large.h5')
