...
BATCH_SIZE = 64
EPOCHS = 500
NUM_KEYPOINTS = 15 * 2  # 15 pairs each having x and y coordinates
...
sample_keypoints = sample_keypoints[:4].reshape(-1, 15, 2) * IMG_SIZE
...
sample_val_keypoints = sample_val_keypoints[:4].reshape(-1, 15, 2) * IMG_SIZE
predictions = model.predict(sample_val_images).reshape(-1, 15, 2) * IMG_SIZE
